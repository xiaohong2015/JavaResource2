...
public class Change {
    ...
    public static String check(int length,String INDate){
    	...
    	String separate="-";
    	if (INDate=="")
    		return "office.time.YMD.not.wanzheng";
    	
    	if(INDate.indexOf(separate,0)==-1)return "office.time.format.queshao.separate";
    	
    	subDate=INDate.substring(0,INDate.indexOf(separate));
    	if(subDate.length()!=length)
    	     return "office.time.YMD.length.not.right";
    	if(strtoint(subDate)==-1)
    		return "office.time.YMD.context.not.number";
    ...
    public static String checkdate(String INDate){
    	...
    	String separate="-";
    	String return_str="office.meet.notime.error";
    	...
    	if(subDDD==-1)
     	...
     	if(subMMM==2){
    	   if((subYYY%4==0&&subYYY%100!=0)||subYYY%400==0){
    	       if(subDDD>29) return return_str="office.time.ruinian.2.day.wrong";	
    	   }
    	   else{
    		   if(subDDD>28) return return_str="office.time.not.ruinian.2.day.wrong";
    	   }
    	}
    	else{
    		if(subMMM==1||subMMM==3||subMMM==5||subMMM==7||subMMM==8||subMMM==10||subMMM==12){
    			if(subDDD>31) return return_str="office.time.31.day.wrong";
    		}
    		else{
    			if(subDDD>30) return return_str="office.time.30.day.wrong";
    		}
    	}
